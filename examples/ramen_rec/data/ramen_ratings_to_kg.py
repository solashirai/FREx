...
def convert():
    ...
    """
    random.seed(1)  # seed random var, since we're just using random to tack on a fake 'price' to ramens.
    ramen_data = []
    ...
    for ramen_content in ramen_data:
        ...
        output_graph.add(
            (
                ramen_ns[ramen_content[0]],
                ramen_onto_ns["price"],  # a random 'price' which will be used to demonstrate some constraints
