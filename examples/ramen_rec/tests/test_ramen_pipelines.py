...
def test_generate_candidates(
    ...
    candidates = list(
        ramen_candidate_generator(context=RamenContext(target_ramen=test_ramen_101))
    )
...
def test_score_rating(test_ramen_101, test_ramen_202, test_ramen_103, rating_scorer):
    ...
    assert (
        rating_scorer.score(candidate=cand_202) == cand_202.domain_object.rating / 5
        and rating_scorer.score(candidate=cand_103) == cand_103.domain_object.rating / 5
    )
...
def test_filter_prohibited_country(
    ...
    cand_202 = placeholder_ramen_candidate(
        test_ramen_202, context=test_ramen_eater_01_context
    )
    cand_101 = placeholder_ramen_candidate(
        test_ramen_101, context=test_ramen_eater_01_context
    )
...
def test_score_likes_brand(
    ...
    assert likes_brand_scorer.score(candidate=cand_101) == (
        False,
        0,
    ) and likes_brand_scorer.score(candidate=cand_1011) == (
        True,
        0.9,
    )
...
def test_score_likes_country(
    ...
    assert likes_country_scorer.score(candidate=cand_101) == (
        True,
        1,
    ) and likes_country_scorer.score(candidate=cand_1011) == (
        False,
        0,
    )
